  ...
  const getFilteredSidebarLinks = () => {
    ...
    if (role.includes("sale")) {
      return sidebarLinks.filter((link) =>
        ["dashboard", "customers", "sales executive", "order management", "requests"].includes(
   ...
   if (role.includes("invoice") || role.includes("bill")) {
      return sidebarLinks.filter((link) =>
        ["dashboard", "customers", "invoice executives", "price approval control", "requests"].includes(
          link.name.toLowerCase().trim()
        )
      );
    }

    return []; // Unknown role: show nothing
  };
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-white shadow-md flex flex-col">
        ...
        <nav className="flex-grow">
          {filteredSidebarLinks.map(({ name, icon, submenu }) => {
              ...
              <div key={name}>
                <button
                  ...
                  className={`flex items-center justify-between w-full px-6 py-3 hover:bg-blue-100 ${
                    location.pathname.includes(path) || isExpanded
                ...
                >
                  ...
                  {submenu && <span className="ml-2 text-sm">{isExpanded ? "▲" : "▼"}</span>}
                </button>
